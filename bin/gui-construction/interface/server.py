...
#            return jsonify(redis_cache[key_list]
            r = app.response_class(response=json.dumps(redis_cache[key_list]),mimetype='application/json')
            return r
        else:
...
def handle_get_all_redis_keys():
    ...
    keys.sort()
    return app.response_class(response=json.dumps(keys),mimetype='application/json')
